			...
			@foreach($activa as $p)
				@if(($printed==true) and ($p->level_account==2))
					<tr>
						...
						@if	(floatval($saldo1)>=0){
							<td align="right"><strong>{{number_format(floatval($saldo1),2,',','.')}}</strong></td>
						@else
							<td align="right" style="color:red"><strong>{{number_format(floatval($saldo1),2,',','.')}}</strong></td>
						@endif
						@if	(floatval($saldo2)>=0){
							<td align="right"><strong>{{number_format(floatval($saldo2),2,',','.')}}</strong></td>
						@else
							<td align="right" style="color:red"><strong>{{number_format(floatval($saldo2),2,',','.')}}</strong></td>
						@endif
				...
				<tr>
					...
					@elseif ($p->level_account =='3')         
						<td style="padding-left: 1em" colspan="3"><strong>{{$p->account_no}} - {{$p->account_name}}<strong></td>
					@else
